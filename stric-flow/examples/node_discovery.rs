...
fn main() {
    println!("Initializing Kademlia Routing Table for local_node...");
    ...
    println!("Inserting nodes: peer_1, peer_2, peer_3...");
    table.update("peer_1".to_string(), addr_1);
    table.update("peer_2".to_string(), addr_2);
    table.update("peer_3".to_string(), addr_3);

    println!("Searching closest node to 'peer_1'...");
    let closest = table.find_closest_nodes("peer_1", 1);
    
    assert!(!closest.is_empty());
    println!("Closest discovered node ID: '{}', Address: '{}'", closest[0].node_id, closest[0].addr);
    
    assert_eq!(closest[0].node_id, "peer_1");
    assert_eq!(closest[0].addr, addr_1);

    println!("SUCCESS: Kademlia XOR routing table discovery behaves correctly!");
}
